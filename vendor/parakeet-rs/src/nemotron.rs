...
pub struct Nemotron {
    ...
    /// Optional decode-time hook; `None` (the default) leaves decoding unchanged.
...
impl Nemotron {
    ...
    /// the greedy decode loop. Per instance: decoder state is per stream.
    pub fn set_logits_processor(&mut self, p: Option<Box<dyn LogitsProcessor>>) {
        self.logits_processor = p;
    }
    ...
    pub fn reset(&mut self) {
        ...
        if let Some(p) = self.logits_processor.as_mut() {
            p.on_reset();
        }
    }
    ...
    fn decode_chunk(&mut self, encoder_out: &Array3<f32>, enc_frames: usize) -> Result<Vec<usize>> {
        ...
        for t in 0..enc_frames {
            ...
            for _ in 0..max_symbols_per_step {
                let (mut logits, new_state_1, new_state_2) = model.run_decoder(
                    &frame,
                    self.last_token,
                    &self.state_1,
                    &self.state_2,
                ...
                if let Some(p) = self.logits_processor.as_mut() {
                    if let Some(s) = logits.as_slice_mut() {
                        p.process(s, self.blank_id);
                    }
                }
